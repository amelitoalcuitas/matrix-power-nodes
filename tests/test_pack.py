...
class PackWiringTests(unittest.TestCase):
    def test_declared_nodes_are_registered(self):
        ...
        for node_id, module in {'MATRIX_DatasetConfig': 'matrix_datasetconfig', 'MATRIX_DatasetImage': 'matrix_datasetimage'}.items():
    ...
    def test_resolved_blocks_are_present_and_parse(self):
        for slug in ['auth_provider_key', 'spend_admission', 'errors_taxonomy', 'transport_http', 'media_reference_set', 'media_image_in', 'submit_billable', 'submit_sync', 'poll_task', 'media_image_out', 'cache_semantic', 'route_multi', 'ui_progress', 'flow_api_media', 'ui_key_mask', 'ui_appearance', 'ui_model_switch']:
